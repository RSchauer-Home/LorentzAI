...
# ╔═╡ 6b31ec1e-09b6-11ec-0d2b-fd51270d241a
begin
...
# ╔═╡ ec26c964-497f-42fc-ab35-7f0dd262c812


# ╔═╡ 36ceafe1-c39c-4819-99cc-3948adb14a78
...
# ╔═╡ 496737a4-49f8-4454-af94-4a45f4dacfb6
begin
	function min_difference(values)
		return min( [ abs(v1-v2) for v1 in values for v2 in values if v1-v2 != 0 ]...)
	end
	
	function min_distance_diffs(coordinates)
		Xs = [coord[1] for coord in coordinates]
		Ys = [coord[2] for coord in coordinates]
		return (min_difference(Xs),min_difference(Ys)) 
	end
end

# ╔═╡ 3445dadd-281c-49c6-b005-c4dbbfe17b09
# TODO: Add autoremove of long arrows

function plot_DGlvectorfield!(dgl::Function, coordinates::Vector{Tuple{Float64,Float64}}, scale=1.0, autoremove=true)
	
	total_flows = Dict(coord => scale*dgl(coord) for coord in coordinates) 
	min_x_diff, min_y_diff = min_distance_diffs(coordinates)
	viewable_flows = Dict( coord => flow for (coord, flow) in total_flows if (abs(flow[1]) < min_x_diff) &  (abs(flow[2]) < min_y_diff) )
	
	
	# currently only for 2D
	Xs = [coord[1] for (coord, flow) in viewable_flows]
	Ys = [coord[2] for (coord, flow) in viewable_flows]
	flow_Xs = [flow[1] for (coord, flow) in viewable_flows]
	flow_Ys = [flow[2] for (coord, flow) in viewable_flows]
	#quiver(Xs, Ys, flow_Xs,flow_Ys,arrowscale=0.1)
	#quiver_test(Xs, Ys, flow_Xs,flow_Ys)
	quiver(Xs, Ys, quiver=(flow_Xs,flow_Ys)) 
	# quiver expects a function Xs,Ys,f(x,y)
	#   or Xs,Ys,fXs,fYs
end

# ╔═╡ df2d4660-9645-4603-ada6-1e6048fb190d
#plot(solved)

# ╔═╡ 547b5570-e487-4572-8657-1bbc931b2b02
begin
	parameters=[1.1,0.4,0.1,0.4] #example values from wikipedia
	u0 = [5.0,5.0]   
	tspan = (0.0,500)
end

# ╔═╡ cdf3d801-2793-49f2-96af-6f405e8b38eb
function parameterized_lotka_volterra!(u,p,t)
    🐰,🐺 = u
    a,b,d,g = p
    d🐰 = a*🐰 - b*🐰*🐺
    d🐺 = d*🐰*🐺 - g*🐺
...
# ╔═╡ db16613c-8a8d-4ffc-b2f9-c44d833b8062
begin
	# use tuples (fixed size!) for performance)
	coords= [ (x,y) for x in range(0.0, 12.0,length=20) for y in range(0.0, 6.0,length=20)]
	
	
	specific_lotka_volterra(u) = parameterized_lotka_volterra!(u,parameters,0) 
end

# ╔═╡ 773663c0-8d45-4dec-9098-6ebc37879f65
prob = DifferentialEquations.ODEProblem(parameterized_lotka_volterra!,u0,tspan,parameters)

# ╔═╡ 938127cb-f6fb-47d4-9860-c514a5d712c6
solved=solve(prob)

# ╔═╡ e9fb9516-135e-4c53-b024-31e174c1a0c7
begin
	pgfplotsx()
	#pyplot() #
	#gr()
	#plotlyjs()
	#gr(size=(600,400))
	plot_DGlvectorfield!(specific_lotka_volterra, coords, 0.075)
	
	traj = [solved(t) for t in range(tspan[1],tspan[end],length=2500)]
	plot!([u[1] for u in traj],[u[2] for u in traj])
end
...
git-tree-sha1 = "d3dc5928b0fb675ab4bbf3a825f887b9fbc60bcf"
...
git-tree-sha1 = "ca3f7f8b708d713b2a96735153282a65b1c1c466"
